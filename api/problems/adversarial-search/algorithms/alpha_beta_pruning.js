(SolverBase, game) => {
    ...
    class AlphaBeta extends SolverBase {
        constructor(game) {
            super(game);
        }
        ...
        *runAlgorithm(root) {
            yield* this.dfsAlphaBeta(root, Number.MIN_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
        ...
        *dfsAlphaBeta(node, alpha, beta) {
            ...
            let bestMoves = [];
            for (let move of node.moves) {
                yield* this.dfsAlphaBeta(move.position, visited);
